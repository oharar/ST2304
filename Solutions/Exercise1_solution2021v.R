...
#' 
#' <span style="color:purple"> Questions appear in purple or R code. </span>
#' 
...
#' 
#' <span style="color:Purple">Answer in R code </span>
#' 
#+ warning = FALSE, error = FALSE, include = TRUE, echo = TRUE, eval = TRUE
...
#' 
#' <span style="color:Purple">Answer to A3i and A3ii below in R code. </span>
#' 
#+ warning = FALSE, error = FALSE, include = TRUE, echo = TRUE, eval = TRUE
...
#' equation**) </span>
#' 
#' <span style="color:purple"> We can differentiate the likelihood, 
#' and find the sex ratio where the slope is zero.
#' We know from last week you can use: </span>
#' 
#' <span style="color:purple"> $\hat{p} = \frac{r}{N}$ </span>
#' 
#' <span style="color:purple"> or we can use `optimise()`. </span>
#' 
...
#' likelihoods.
#' 
#+ warning = FALSE, error = FALSE, include = TRUE, eval = TRUE, echo=TRUE
...
#+ warning = FALSE, error = FALSE, include = TRUE, eval = TRUE
...
#' 
#' <span style="color:purple"> The maximum likelihood estimate alone is not
#' enough because it does not tell us about the variability in our result. 
#' When we did simulations you can see each time you take a sample and 
#' make an estimate for the parameter, you get a different result. This
#' variation needs to be considered when making statistical 
#' conclusions. So, we want a measure of uncertainty (variance of the 
#' sampling distribution of a statistic/parameter), standard errors and
#' confidence intervals are ways we can do this. </span>
#'
...
#' 
#' <span style="color:purple"> The standard error is the standard deviation 
#' of the 
...
#' But should include some reference to the sampling distribution. </span>
...
#' 
#' <span style="color:purple"> The CI for probability of being female is 
#' 0.34 to 0.54, 
#' which includes 0.5. So, 50% probability of being female is within the plausible
#' range for this parameter based on our estimation. 
#' So, there seem to be as many females as would be expected 
#' from a 50:50 ratio, even though the MLE is less than 0.5, when we include
#' the necessary uncertainty, 0.5 is within the interval.
#' You do not need the MLE, which is 0.44, to reach the conclusion.</span>
#' 
#' -----
#' 
...
#' 
#' <span style="color:purple"> The confidence intervals are 95% confidence intervals
...
#' <span style="color:purple"> This means that based on our results, it is unlikely that the
#' true population abundance is greater than 40 but it could be less than 18. 
#' We are not in danger of too many dragons. 
#' But the population could be in danger of collapse.</span>
...
#' 
#' <span style="color:purple"> The approximate intervals do not cover 
#' 95% of the Poisson distribution. 
#' Particularly the lower CI is too low in the approximation. 
#' The approximate intervals cover > 95% of the Poisson so they are
#' over-representing uncertainty, which is probably better than under-estimating. 
#' But, it is very close
#' so also ok to think they are fine. The approximation is also a lot easier to
#' use. </span>
#' 
#' 
#' -----
...
#' 
#' <span style="color:purple"> We are expecting that you should want to increase 
#' the population slightly. The MLE
...
#' <span style="color:purple">Whatever management strategy you choose should 
#' include discussion of the 
#' upper and lower bounds of the confidence intervals when justifying the answer.</span>

#' <span style="color:purple"> You have information on sex ratio. 
#' Which is ok, 50:50 was within the plausible
...
#' but you must be aware that you cannot say much about anything we have no data on. </span>

#' <span style="color:purple"> Anything biologically sensible is accepted here for how to improve. 
