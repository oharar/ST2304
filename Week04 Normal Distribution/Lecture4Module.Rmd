---
title: 'Week 4: The Normal Distribution'
output:
  html_document:
    df_print: paged
---
...
```{r ReadPuxPhil, echo=TRUE}
# read in the functions for this module
source("../Functions/NormalDistFunctions.R")
# Read the data
...
- `rnorm()` - this simulates from a normal distribution
- `dnorm()` - this calculates the density of a normal distribution for values `x`. 
- `pnorm()` - this calculates the cumulative density of a normal distribution for values `x`. This is the probability that a random draw from the distribution would be less than `x`.
- `qnorm()` - this calculates the values of `x` that would be the `q`th quantile

<span style="color:blue">Simulate 1000 data points from a normal distribution with the same mean and standard deviation as the temperature data, using `rnorm()`.</span>

```{r rnorm, echo=TRUE, eval=F, fig.height=3}
...
<span style="color:blue">Calculate the density data from the distribution for values using `dnorm()`, but for more than 4 points.
    - why is the density different from the probability (hint: the normal is continuous)</span>
    
    ```{r hidden, eval=T, echo=F, fig.height=3}
...
```{r dnorm, eval=F, echo=TRUE, fig.height=3}
# First, create a sequence of numbers to calculate the density on
At.data <- seq(min(GDay$Temperature), 
               max(GDay$Temperature), length=4)
dens.data <- dnorm(At.data, mean = MeanTemp, sd = sdTemp)
...
<span style="color:blue">Calculate the cumulative density data from the distribution for values using `pnorm()`.</span>

```{r pnorm, eval=F, echo=TRUE, fig.height=3}
cumul.data <- pnorm(At.data, mean = MeanTemp, sd = sdTemp)
plot(At.data, cumul.data, type="l")
```

<span style="color:blue"> Calculate the 2.5th, 25, 50th, 75th, and 97.5th quantiles from the distribution using `qnorm()`</span>
```{r qnorm, eval=F, echo=TRUE, fig.height=3}
Quantiles <- c(0.025, 0.25, 0.5, 0.75, 0.975)
quants.data <- qnorm(Quantiles, 
                     mean = MeanTemp, sd = sdTemp)
...
# Simulate data sets with mean 1, standard deviation 2 and 10 data points
# For each one calcualte the MLE for the mean
Sims <- SimNormMeans(mu=1, sigma=2, N=10, nReps = 1e4)
hist(Sims)
...
- <span style="color:blue">Use these and `SimNormMean()` to simulate replicate data (with the same number of data points as in the original data), and estimate the means of each simulated data point</span>
...
# Use this function to calculate the confidence inteval for the mean.
...
<span style="color:blue">Calculate the 95% confidence interval for the mean of the Patuxwarney temperature data.</span>
...
<span style="color:blue">Run more than 5 simulations and look at the likeihood for the difference. What is the mean and the 95% confidence interval?</span>
