...
#' increased agression. 50% of the population have already been infected. It is not known how many are immune.
#' 
...
# between two groups is different to zero.
...
#+ warning = FALSE, error = FALSE, include = FALSE, echo = TRUE, eval = FALSE
...
#' 
#+ warning = FALSE, error = FALSE, include = FALSE, echo = FALSE
# Lower bound is 1.86 and upper is 2.49 seconds. They do not span zero. 
...
# is that humans are faster. They are faster by an average of 0.99 seconds
# with 95% confidence the mean is between 0.62 and 1.38 seconds. 
# But you may choose to stay, because 0.6 seconds or even just over
# 1 second, is not a big difference!!!
...
# the higher uncertainty in small sample sizes.
...
#' 
#+ warning = FALSE, error = FALSE, include = FALSE, echo = FALSE
# Two the mean and the standard deviation/variance
...
#' question A4. But we will print them below too. 
#' 
#+ warning = FALSE, error = FALSE, include = FALSE, echo = FALSE
# The Normal confidence intervals are narrower than those calculated
...
#' 
#+ warning = FALSE, error = FALSE, include = FALSE, echo = FALSE
# It is not a great model here because the confidence intervals
# are too tight. They under estimate the uncertainty because we 
# have only a few data points. The T distribution captures the 
# uncertainty better. 

# The Normal distribution will be more appropriate with more data. 
# You can always test this out but creating more data and running
# all the code again. 
...
#' 
#+ warning = FALSE, error = FALSE, include = FALSE, echo = FALSE
# Using the Normal distribution confidence intervals should not
# change your decision. Both sets of CIs do not cross 0 so we are
# 95% confident in both cases that there is a non-zero difference
# in 100m times before and after infection.
# The difference is very minimal. You might be slightly more likely to
# run using normal distribution confidence intervals.
...
# - Wind data, this could influence the change in times (problem).
# - More data so we can be more confident of results (data).
# - Data from another time, maybe there is an initial increase then 
# it drops off (data).
# - Could be other reasons for the increase e.g. warmer (problem).
