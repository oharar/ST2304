...
We can read in the data, and plot the difference variables against each other.
...
Now we have two covariates, so we ewant to fit a regression model with both covariates in it. The extension to more than 2 is straightforward.
...
Before we had a model with a single covariate, so the model was a line. Now we have two covariates, so the model is a 2D extension of a line, i.e. a plane. We can see that (and the data) below 
...
surf <- add_surface(plot_ly(z = ~surface))
surf
...
The full model is a plane (or a hyper-plane if we have more than two covariates), but for this model we can make things simpler. The model that was fitted was 

$$
y_i = \color{red}{\hat{\alpha} + \hat{\beta_{1}} x_{i1} + \hat{\beta_{2}} x_{i2}} + \color{blue}{\varepsilon_i}
$$
($x_{i1}$ is Body Size, $x_{i2}$ is Gape Size. The hats on Greek letters show that we are using the estimates of the parameters). So if we want to know the effect of body size, we can ask how much it changes, 


$$
y_i = \color{grey}{\hat{\alpha} +} \color{red}{\hat{\beta_{1}} x_{i1}} \color{grey}{+ \hat{\beta_{2}} x_{i2} + {\varepsilon_i}}
$$

If we look at this, we can see that if we keep $x_{i2}$ constant, the model is 
$y_i = \color{grey}{\hat{\alpha} + \hat{\beta_{2}} x_{i2}} + \color{red}{\hat{\beta_{1}} x_{i1}} + \color{blue}{\varepsilon_i}$, so we can think of it as a simple linear regression with an intercept of $\hat{\alpha} + \hat{\beta_{2}} x_{i2}$, i.e. a straight line. Of course, if we change $x_{i2}$ we change this, but the slope is the same.

Visually, fixing $x_{i2}$ is taking a slice through the 3D plot: the value of $x_{i2}$ determines where we make teh slice. If we fix $x_{i1}$ instead, we are slicing at 90&deg;.

In practice, this means that for these simple models we can interpret the parameter as we would in a simple regression: they tell us how much the response (e.g. dust eaten) is affected if the covariate (e.g. gape size) changes by 1.
...
So far we have not been thinking about the intercept. It is a necessary part of the model, but is not so easy to interpret. Often it is largely ignored, even though it still has to be in the model.
...
Sometimes we can find it helpful to have an intercept that is interpretable, if only so we can do a sanity check. We can easily move the intercept like this:
...
There are technical reasons for doing this, but those are handled by the software. For our purposes, this can be useful to make sure the model is sensible, and to understand why you might see a strange value of the intercept. When we look at categorical variables, we will be thinking more about the intercept, and what it means.
