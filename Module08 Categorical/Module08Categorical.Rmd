...
From the summary we can see that the Fertilised effect is positive, i.e. the so the mean for the Fertilised effect is larger than the intercept (the mean for the Control): the estimated difference is `r round(coef(modFC)[2], 2)`t/ha. The standard error is  `r round(summary(modFC)$coefficients[2,2], 2)`t/ha, so the effect is clearly a long way from 0. Indeed the confidence interval is `r round(confint(modFC)[2,1], 2)` t/ha - `r round(confint(modFC)[2,2], 2)` t/ha.
...
If it helps (and it may or may not), these are the unique combination of factor levels in the design matrix. Each row is one observation, but in the data there are several observations with the same 1s and 0s as row 31, for example.
